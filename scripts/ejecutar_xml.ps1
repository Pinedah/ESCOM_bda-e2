# Script de Ejecucion - Modelo XML (PostgreSQL)

Write-Host "Ejecutando Modelo XML (PostgreSQL)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "1. Creando estructura XML..." -ForegroundColor Yellow

# Ejecutar scripts en orden
$scripts = @(
    "modelo_xml/schema_xml.sql",
    "modelo_xml/poblar_datos_xml.sql"
)
...
if ($success) {
    Write-Host ""
    Write-Host "2. Ejecutando consultas XML..." -ForegroundColor Yellow
    $consultasPath = Join-Path (Get-Location) "modelo_xml/consultas_xml.sql"
    Execute-SqlFile -FilePath $consultasPath -DatabaseName $DB_NAME | Out-Null
    
    Write-Host ""
    Write-Host "MODELO XML EJECUTADO EXITOSAMENTE!" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "ERROR: Fallos durante la ejecucion del modelo XML" -ForegroundColor Red
    exit 1
}
