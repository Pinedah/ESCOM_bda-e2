# Script de Ejecucion - Modelo Multidimensional (PostgreSQL)

Write-Host "Ejecutando Modelo Multidimensional (PostgreSQL)..." -ForegroundColor Cyan

# Variables de configuracion
$DB_HOST = "localhost"
$DB_PORT = "5432"
$DB_NAME = "peliculas_dwh"
$DB_USER = "postgres"
$DB_PASS = "panke"
...
Write-Host ""
Write-Host "1. Creando estructura del Data Warehouse..." -ForegroundColor Yellow

# Ejecutar scripts en orden
$scripts = @(
    "modelo_multidimensional/schema_multidimensional.sql",
    "modelo_multidimensional/poblar_dimensiones.sql", 
    "modelo_multidimensional/poblar_hechos.sql"
)
...
if ($success) {
    Write-Host ""
    Write-Host "2. Ejecutando consultas de negocio..." -ForegroundColor Yellow
    $consultasPath = Join-Path (Get-Location) "modelo_multidimensional/consultas_negocio.sql"
    Execute-SqlFile -FilePath $consultasPath -DatabaseName $DB_NAME | Out-Null
    
    Write-Host ""
    Write-Host "MODELO MULTIDIMENSIONAL EJECUTADO EXITOSAMENTE!" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "ERROR: Fallos durante la ejecucion del modelo multidimensional" -ForegroundColor Red
    exit 1
}
